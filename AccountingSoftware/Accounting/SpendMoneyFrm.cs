...
namespace AccountingSoftware.Accounting
{
    public partial class SpendMoneyFrm : Form
    {
        ...
        {
            if (this.accDs1.SpendMoney.Rows.Count == 0)
            {
            ...
            try
            {
                ...
                if (sf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    final_doc.Add(title);
                    //--------add subtitle-------
                    iText.Layout.Element.Paragraph sub_title = new Paragraph("Spend Money Report");
                    ...
                    //==============add table data==============================
                    for (int row_index = 0; row_index < this.accDs1.SpendMoney.Rows.Count; row_index++)
                    {
                        this.bindingSource1.Position = row_index;
                        table_data_loader(tbl, this.spendId_txtBox.Text, 1, TextAlignment.CENTER);
                        table_data_loader(tbl, this.customerName_txtBox.Text, 2, TextAlignment.LEFT);
                        table_data_loader(tbl, this.spendDate_dtp.Text, 1, TextAlignment.LEFT);
                        //-------------
                        string sv = "";
                        decimal dc;
        ...
        private void excel_printToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (sf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    wb.AddWorksheet(this.accDs1.SpendMoney, "MySheet");
